...
RE_covtot1{N} = RE0_cov(:,:,1);
RE_covtot2{N} = RE0_cov(:,:,2);
SItot_E(N) = squeeze(RE0(:,1) - RE0(:,2))' * inv(0.5 * (RE_covtot1{N} + RE_covtot2{N})) * squeeze(RE0(:,1) - RE0(:,2)); 
SItot_E_ind(N) =  squeeze(RE0(:,1) - RE0(:,2))' * inv(0.5 * diag(diag(RE_covtot1{N} + RE_covtot2{N}))) * squeeze(RE0(:,1) - RE0(:,2)); 
SI_I(N) = m0I(1,2) / NI;
SI_E(N) = m0E(1,2) / NE;
...
if ~isnan(sum(RE_covtot1{N} + RE_covtot2{N}))

    SItot_E_pseudo(N) = squeeze(RE0(:,1) - RE0(:,2))' * pinv(0.5 * (RE_covtot1{N} + RE_covtot2{N})) * squeeze(RE0(:,1) - RE0(:,2)); 

else
    
    SItot_E_pseudo(N) = nan;
    
end

SItot_E_epsridge(N) = squeeze(RE0(:,1) - RE0(:,2))' * inv(0.5 * (RE_covtot1{N} + RE_covtot2{N} + 2 * eps * eye(1000))) * squeeze(RE0(:,1) - RE0(:,2)); 
