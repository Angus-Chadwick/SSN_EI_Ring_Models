...
theta_aE = (stimvals(1) + stimvals(2))/2;
...
theta_s = stimvals(q);
IE_TD_area = 0.002 * (p1-1);
II_TD_area = 0.01 * (p2-1);
...
parfor n=1:Nloop
    ...
    RE_TD{n}(:,q) = mean(rE_TD(:,1000:end),2);
    RI_TD{n}(:,q) = mean(rI_TD(:,1000:end),2);
      
    RE_std_TD{n}(:,q) = std(rE_TD(:,1000:end),[],2);
    RI_std_TD{n}(:,q) = std(rI_TD(:,1000:end),[],2);
        
    RE_cov_TD{n}(:,:,q) = cov(rE_TD(:,1000:end)');
    Rtot_cov_TD{n}(:,:,q) = cov([rE_TD(:,1000:end); rI_TD(:,1000:end)]')
