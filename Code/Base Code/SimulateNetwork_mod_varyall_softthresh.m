function [rE, rI] = SimulateNetwork_mod_varyall_softthresh(network, inputs, Nt, NoiseModel,R0)
...
elseif strcmp(NoiseModel, 'Add')
    ...
    for t=1:Nt
        ...
        drE = (-rE(:,t) + log(1+exp(dumE)).^gamma) / tauE;
        drI = (-rI(:,t) + log(1+exp(dumI)).^gamma) / tauI;
