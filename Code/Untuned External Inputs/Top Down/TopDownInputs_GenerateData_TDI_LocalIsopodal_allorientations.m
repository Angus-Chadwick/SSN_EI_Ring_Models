...
[~, id]  = min(abs(stimvals(q) - theta_pI));  % centre stimuli on a cell (centred both on an E and an I due to integer NE/NI)
stimvals(q) = theta_pI(id);
...
for iv1 = 1:Nstim
    for iv2 = 1:Nstim
        ...
        m0E_TD(iv1,iv2) = nansum(SI0E_TD);

        % I selectivity
        
        dmu_0I_TD = RI0_ang_TD - RI0_vert_TD;
        
        poolvar_0I_TD = (0.5* (RI0_std_ang_TD.^2 + RI0_std_vert_TD.^2));

        SI0I_TD = dmu_0I_TD.^2 ./ poolvar_0I_TD;

        m0I_TD(iv1,iv2) = nansum(SI0I_TD);
